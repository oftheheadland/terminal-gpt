...
async function queryChatGPT(instruction, apiKey) {
  ...
  const prompt = `Write a ${osName} command that does ${instruction}. Only return the command and nothing else. It should be able to directly run in the command prompt or terminal. Do not include explanations, comments, or any additional text.
  If you cannot accomplish the task with the command, return "404". 
...
async function runScript(script) {
  exec(script, (error, stdout, stderr) => {
    ...
    stdout.length > 0 ? console.log(`\nScript output: ${stdout}`) : console.log('\nScript completed successfully!')
  })
}
